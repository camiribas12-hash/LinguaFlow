...
function SReview({ due, reviews, onReview, toast, profile }) {
  ...
  const speakCard = (c, isBack) => {
    ...
    if (isBack) {
      ...
      const eng = lines.find(l => /[.!?,]/.test(l) && !/^[\uD83C-\uDBFF\uDC00-\uDFFF]/.test(l.trim()))
      text = (eng || c.front || '').replace(/^[^a-zA-Z]+/, '').trim()
      if (!text) text = (c.front || '').replace(/^Corrija:\s*/i, '').trim()
    } else {
      text = (c.front || '').replace(/^Corrija:\s*/i, '').trim()
    }
    if (!text) return
    ...
    <div onClick={() => !feedback && setFlipped(!flipped)}
      ...
      {!flipped
        ? <>
            ...
            {!feedback && <button onClick={e => { e.stopPropagation(); speakCard(card, false) }} style={{ background: 'rgba(255,255,255,.15)', border: '1.5px solid rgba(255,255,255,.3)', borderRadius: 20, padding: '6px 14px', color: '#f3e6d2', fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>🔊 Ouvir</button>}
            {!feedback && <div style={{ fontSize: 12, color: 'rgba(243,230,210,.5)' }}>👆 Toque para revelar</div>}
          </>
        : <>
            <div style={{ fontSize: 13, color: 'rgba(243,230,210,.9)', lineHeight: 1.9, whiteSpace: 'pre-wrap', textAlign: 'left', width: '100%' }}>{card.back}</div>
            {!feedback && <button onClick={e => { e.stopPropagation(); speakCard(card, true) }} style={{ background: 'rgba(255,255,255,.15)', border: '1.5px solid rgba(255,255,255,.3)', borderRadius: 20, padding: '6px 14px', color: '#f3e6d2', fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, alignSelf: 'center' }}>🔊 Ouvir exemplo</button>}
          </>
      }
    </div>
