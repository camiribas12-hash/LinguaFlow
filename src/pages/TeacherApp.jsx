...
function TSync({ profile, students, reload, toast }) {
  ...
  const doSync = async () => {
    ...
    addLog('🔄 Conectando ao Notion...')
    try {
      const lessons = await fetchNotionLessons()
      addLog(`✅ ${lessons.length} aula(s) encontrada(s)`, 'success')
      let newL = 0, newC = 0
      for (const nl of lessons) {
        const student = students.find(s =>
          nl.student_name && (s.name.toLowerCase().includes(nl.student_name.toLowerCase()) || nl.student_name.toLowerCase().includes(s.name.split(' ')[0].toLowerCase()))
        ) || students[0]
        if (!student) { addLog(`⚠️ Aluno não encontrado para "${nl.topic || nl.lesson_name}"`, 'warning'); continue }
        const { data: existing } = await supabase.from('lessons').select('id').eq('topic', nl.lesson_name || nl.topic).eq('student_id', student.id).single()
        if (existing) { addLog(`⏭️ "${nl.lesson_name || nl.topic}" já existe`); continue }
        const { data: lessonRow } = await supabase.from('lessons').insert({ teacher_id: profile.id, student_id: student.id, date: nl.date || today(), topic: nl.lesson_name || nl.topic || 'Aula', duration: 60, summary: nl.topic || '', transcript: nl.transcription || '', processed: true }).select().single()
        if (!lessonRow) continue
        const contentItems = []
        if (nl.vocabulary) nl.vocabulary.split(',').filter(Boolean).forEach(w => { const word = w.trim(); if (word) contentItems.push({ lesson_id: lessonRow.id, student_id: student.id, teacher_id: profile.id, type: 'vocabulary', word, definition: '', translation: '', example: '' }) })
        if (nl.grammar?.trim()) contentItems.push({ lesson_id: lessonRow.id, student_id: student.id, teacher_id: profile.id, type: 'grammar', word: nl.grammar.trim(), definition: nl.grammar, translation: '', example: '' })
        if (contentItems.length) {
          const { data: saved } = await supabase.from('content').insert(contentItems).select()
          if (saved?.length) {
            const cards = saved.map(item => ({ content_id: item.id, student_id: student.id, type: item.type, front: item.word, back: `📖 ${item.definition || item.word}\n\n${item.translation ? '🇧🇷 ' + item.translation : ''}`.trim() }))
            await supabase.from('flashcards').insert(cards)
            newC += saved.length
          }
        }
        newL++
        addLog(`✅ "${lessonRow.topic}" → ${student.name} | ${contentItems.length} itens`, 'success')
      }
      addLog(`🎉 Concluído: ${newL} novas aulas, ${newC} itens de conteúdo`, 'success')
      await reload()
      toast({ type: 'success', title: 'Sync concluído!', msg: `${newL} novas aulas importadas` })
    } catch (e) { addLog('❌ Erro: ' + e.message, 'error'); toast({ type: 'error', title: 'Erro no sync', msg: e.message }) }
    setLoading(false)
  }
  ...
  return <div>
    <div style={{ ...crd, marginBottom: 18 }}>
      <h3 style={{ fontSize: 15, fontWeight: 700, color: C.green, margin: '0 0 8px' }}>🔄 Sincronizar do Notion</h3>
      <p style={{ fontSize: 13, color: C.muted, margin: '0 0 16px', lineHeight: 1.6 }}>Importa aulas do banco <strong style={{ color: C.tx }}>All Lessons</strong> no Notion. Cria conteúdo e flashcards automaticamente para cada aluno.</p>
      <button onClick={doSync} disabled={loading} style={{ ...bp(C.sage.replace('#', '') === '9baf8b' ? C.green : C.green), opacity: loading ? .7 : 1, display: 'flex', alignItems: 'center', gap: 8 }}>{loading ? <Spin /> : '🔄'}{loading ? 'Sincronizando...' : 'Sincronizar Agora'}</button>
    </div>
